...
const Carousel = (props: CarouselProps) => {
    ...
    useEffect(() => {
        const next = () => {
            if (animating) return; // If already animating, exit early
        
            setAnimating(true);
            const newIndex = index + 1 === props.items.length ? 0 : index + 1;
            setIndex(newIndex);
            setDirection(1);
    ...
    }, [animating, index, props.interval, props.items.length]);
    ...
    const previous = () => {
        ...
        setAnimating(true);
        const newIndex = index - 1 < 0 ? props.items.length - 1 : index - 1;
        setIndex(newIndex);
        setDirection(-1);
